...
import { Mic, PlayCircle, Clock, TrendingUp, AlertCircle } from 'lucide-react';
...
const CAT_COLOUR: Record<string, string> = {
  behavioural: 'bg-blue-500/10 text-blue-500',
  technical:   'bg-violet-500/10 text-violet-500',
  case:        'bg-amber-500/10 text-amber-500',
  culture:     'bg-emerald-500/10 text-emerald-500',
};

// Static performance breakdown — would come from session history in a real backend
...
export default function InterviewCoachPage() {
  ...
  const result       = useMemo(() => computeInterviewReadiness(profile), [profile]);
  const [activeType, setActiveType] = useState<string>('All');

  const categories = ['All', 'behavioural', 'technical', 'case', 'culture'];
  const questions   = activeType === 'All'
    ? result.questions.slice(0, 3)
    : result.questions.filter(q => q.category === activeType).slice(0, 3);

  // Session history uses profile-derived company names
  const companies = result.targetCompanies;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border/50 rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-foreground">{result.readinessScore}%</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div>
                ...
                <select className="w-full bg-muted/50 border border-border/50 rounded-md text-sm p-2.5 outline-none focus:border-primary/50">
                  <option>{result.targetRole}</option>
                  <option>Head of {profile.sector}</option>
                  <option>Director — {profile.sector}</option>
                </select>
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Company (Optional)</label>
                <select className="w-full bg-muted/50 border border-border/50 rounded-md text-sm p-2.5 outline-none focus:border-primary/50">
                  <option>Generic {profile.sector} (UAE)</option>
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-medium text-sm flex items-center gap-2">
                      {companies[0] ?? 'Top Employer'} - {result.targetRole}
              ...
              <div className="flex items-center justify-between p-4 rounded-lg border border-border/50 bg-muted/10 hover:bg-muted/20 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-medium text-sm">{companies[1] ?? 'Top Employer'} - Senior Role</h4>
              ...
              <div className="flex items-center justify-between p-4 rounded-lg border border-border/50 bg-muted/10 hover:bg-muted/20 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-medium text-sm">{companies[2] ?? 'Top Employer'} - Product Case</h4>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border/50 rounded-xl p-6">
                ...
                {categories.map(c => (
                  <button
                    ...
                    className={`text-[10px] px-1.5 py-0.5 rounded transition-colors capitalize ${
                      activeType === c ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground'
                    }`}
                  >
            ...
            <div className="grid grid-cols-1 gap-3">
              {questions.map(item => (
                <div key={item.id} className="p-3 border border-border/50 rounded-lg hover:border-primary/30 transition-colors cursor-pointer group">
                  <p className="text-xs font-medium leading-snug mb-2 group-hover:text-primary transition-colors">{item.question}</p>
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded capitalize ${CAT_COLOUR[item.category] ?? 'bg-muted text-muted-foreground'}`}>
                      {item.category}
                    </span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium capitalize ${DIFF_COLOUR[item.difficulty] ?? 'bg-muted text-muted-foreground'}`}>
                      {item.difficulty}
                    </span>
                  </div>
                </div>
