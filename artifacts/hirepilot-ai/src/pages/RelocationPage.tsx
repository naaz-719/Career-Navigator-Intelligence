...
export default function RelocationPage() {
  ...
  const [toCountry, setToCountry] = useState(
    profile.targetCountries[0] ?? "United Arab Emirates",
  );
  ...
  const savingsBarData = result.toCountries.map((c) => ({
    ...
    savings: Math.max(
      0,
      profile.targetSalary - (COST_REF[c.name]?.total ?? 8000),
    ),
    tax: 0,
  ...
  const savingsMultiplier =
    result.fromCountry === toCountry
      ? 1
      : (
          savings / Math.max(1, profile.currentSalary - ref.total * 0.6)
        ).toFixed(1);
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-1 mb-6">
        ...
        <p className="text-muted-foreground text-sm">
          The financial truth about moving to the GCC
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-sm border-b border-border/50 pb-2">
              Monthly Financial Estimate (USD)
            </h3>
            <p className="text-xs text-muted-foreground italic">
              Cost-of-living figures are directional estimates, not verified
              market surveys.
            </p>
            <div className="flex justify-between items-center py-2">
              <span className="text-sm text-muted-foreground flex items-center gap-2">
                <TrendingUp className="w-4 h-4" /> Take-home Salary
              </span>
              <div className="flex gap-4 text-sm text-right">
                <div className="w-24 text-muted-foreground line-through">
                  {toUsd(profile.currentSalary)}
                </div>
                <div className="w-24 font-bold text-green-500">
                  {toUsd(profile.targetSalary)}
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center py-2">
              <span className="text-sm text-muted-foreground flex items-center gap-2">
                <Landmark className="w-4 h-4" /> Income Tax
              </span>
              <div className="flex gap-4 text-sm text-right">
            ...
            <div className="flex justify-between items-center py-2">
              <span className="text-sm text-muted-foreground flex items-center gap-2">
                <Home className="w-4 h-4" /> Housing (1BR)
              </span>
              <div className="flex gap-4 text-sm text-right">
                <div className="w-24 text-foreground">
                  ~{toUsd(ref.housing * 0.5)}
                </div>
                <div className="w-24 font-bold text-amber-500">
                  {toUsd(ref.housing)}
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center py-2 border-t border-border/50 mt-2 pt-4">
              ...
              <div className="flex gap-4 text-right">
                <div className="w-24 text-muted-foreground">
                  {toUsd(profile.currentSalary * 0.15)}
                </div>
                <div className="w-24 font-bold text-primary text-xl">
                  {toUsd(savings)}
                </div>
              </div>
            </div>
            <div className="text-xs text-primary/80 bg-primary/10 p-2 rounded text-center font-medium">
              You could save significantly more per month by relocating to{" "}
              {toCountry
                  ...
                  >
                    {savingsBarData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          entry.name === SHORT_COUNTRY[toCountry]
                            ? "hsl(var(--primary))"
                            : "hsl(var(--chart-2))"
                        }
                      />
              ...
              {[
                {
                  ...
                  desc: `${profile.sector} professionals earning AED ${Math.round(profile.targetSalary / 1000)}K/mo`,
                  status: "done",
                },
                {
                  title: "MOHRE Approval",
                  desc: "Ministry of HR clearance (1–2 weeks)",
                  status: "next",
                },
                {
                  title: "Entry Permit",
                  desc: "Issued for 6 months to complete process",
                  status: "pending",
                },
                {
                  title: "Medical & Emirates ID",
                  desc: "Local testing (1 week)",
                  status: "pending",
                },
                {
                  title: "Visa Stamping",
                  desc: `${profile.targetSalary >= 30000 && toCountry === "United Arab Emirates" ? "10-year Golden Visa" : "Employment Visa"} residency granted`,
                  status: "pending",
                },
              ].map((step, i) => (
                <div key={i} className="flex gap-4 items-start relative">
                  <div
                    className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center border-2 z-10 bg-card ${
                      step.status === "done"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div
                key={i}
                className="p-4 border border-border/50 bg-card rounded-xl"
              >
                <div className="flex justify-between items-center mb-3">
                  <div className="font-medium flex items-center gap-1.5">
                    <span className="text-lg">{c.country}</span> {c.city}
                  </div>
