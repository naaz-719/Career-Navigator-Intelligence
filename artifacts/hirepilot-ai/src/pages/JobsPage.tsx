...
export default function JobsPage() {
  ...
  const [activeCountries, setActiveCountries] = useState<string[]>(
    profile.targetCountries.length > 0 ? profile.targetCountries : GCC_COUNTRIES
  );
        ...
        <div className="bg-card border border-border/50 rounded-xl p-4 space-y-5 sticky top-20">
            ...
            <div className="space-y-2">
              {GCC_COUNTRIES.map((c) => (
                <label key={c} className="flex items-center gap-2 text-sm cursor-pointer group">
                  ...
                  />
                  <span className="group-hover:text-primary transition-colors">{FLAG_LABELS[c]}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-border/50">
            ...
            <label className="flex items-center justify-between text-sm cursor-pointer">
              ...
              >
                <span className={`inline-block h-3 w-3 transform rounded-full bg-white transition ${hiddenGhost ? 'translate-x-5' : 'translate-x-1'}`} />
              </button>
            </label>
          </div>
            ...
            >
              ...
              {job.isGhost && (
                <div className="absolute top-0 left-0 right-0 bg-amber-500/10 border-b border-amber-500/20 px-4 py-1.5 flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-amber-500">High Ghost Job Risk Detected ({job.ghostRisk}%) — </span>
                  <span className="text-xs text-amber-500/80">Company has reposted this role multiple times without hiring.</span>
                </div>
              )}
                ...
                <div className="flex gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 text-sm text-muted-foreground mt-1">
                      ...
                      <span className="flex items-center gap-1"><MapPin className="w-3 h-3"/> {job.flag} {job.loc}</span>
                      <span className="hidden md:flex items-center gap-1"><Briefcase className="w-3 h-3"/> {job.sector}</span>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-3">
                      {job.tags.map(t => (
                        <span key={t} className="px-2 py-1 rounded bg-muted/50 border border-border/50 text-xs text-muted-foreground">{t}</span>
                      ))}
                    </div>
                  </div>
                </div>
      ...
      <div className="w-full lg:w-72 flex-shrink-0 space-y-6 hidden xl:block">
        <div className="bg-card border border-border/50 rounded-xl p-5 sticky top-20">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Calibrated for your {profile.sector} profile · {profile.skills.length} skills indexed
          </p>

          <div className="space-y-4 mb-6">
            <div className="p-3 bg-muted/30 rounded-lg border border-border/50">
              <div className="text-xs text-muted-foreground mb-1">Avg Salary · {profile.sector} GCC</div>
              <div className="text-lg font-bold text-primary">AED {Math.round(avgSalary / 1000)}K</div>
