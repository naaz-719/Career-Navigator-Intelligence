...
export default function CareerIntelligencePage() {
  ...
  const demand    = getMarketDemand(profile);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1 mb-6">
        ...
        <p className="text-muted-foreground text-sm">
          AI-powered market analysis for your {profile.sector} profile · {profile.currentRole}
        </p>
      </div>
        ...
        {['market', 'skills', 'nationalization'].map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors capitalize ${
              activeTab === tab ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
            }`}
          >
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-card border border-border/50 rounded-xl p-5">
              ...
              <div className="text-3xl font-bold text-foreground">{demand.avgDays} <span className="text-lg text-muted-foreground">days</span></div>
              <div className="mt-2 text-xs text-muted-foreground">Across GCC · {profile.sector} sector</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={velocityData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="Healthcare" stroke="hsl(var(--chart-4))" fillOpacity={0} strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/50 rounded-xl p-5">
              <h3 className="text-base font-semibold mb-4">Top Hiring · {profile.sector}</h3>
      ...
      {activeTab === 'skills' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-card border border-border/50 rounded-xl p-5 flex flex-col items-center">
            <h3 className="text-base font-semibold mb-2 self-start">Your Skills vs Market Demand</h3>
            <p className="text-xs text-muted-foreground self-start mb-4">
              {profile.skills.length} skills indexed · {profile.sector} sector · {profile.yearsExperience} yrs
            </p>
            <div className="h-[350px] w-full max-w-[420px]">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radar}>
                  ...
                  <Radar name="Your Skills"    dataKey="A" stroke="hsl(var(--primary))" fill="hsl(var(--primary))" fillOpacity={0.35} />
                  <Radar name="Market Demand" dataKey="B" stroke="hsl(var(--accent))"  fill="hsl(var(--accent))"  fillOpacity={0.2}  />
                  <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))', borderRadius: '8px' }} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
            <div className="flex gap-4 text-xs mt-2">
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-sm bg-primary opacity-60" /> Your Profile</div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-sm bg-accent opacity-60" /> Market Demand</div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border/50 rounded-xl p-5">
            ...
            <div className="space-y-4">
              {rankedGaps.slice(0, 4).map((gap, i) => {
                const severity = gap.gap > 40 ? 'Critical' : gap.gap > 20 ? 'High' : 'Medium';
                return (
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Your score: {gap.yourScore} · Market demand: {gap.marketScore}
                        </p>
                      </div>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        severity === 'Critical' ? 'bg-red-500/10 text-red-500' :
                        severity === 'High'     ? 'bg-amber-500/10 text-amber-500' :
              ...
              {rankedGaps.length === 0 && (
                <div className="p-4 rounded-lg border border-emerald-500/20 bg-emerald-500/5 text-center">
                  <p className="text-sm text-emerald-400 font-medium">✓ No significant skill gaps detected</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    Your {profile.skills.length} skills cover the key market demands for {profile.sector}
                  </p>
                </div>
              )}
            </div>
      ...
      {activeTab === 'nationalization' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { country: 'UAE',          flag: '🇦🇪', policy: 'Emiratisation',       risk: 'Low',    ratio: '8% target',           color: 'text-green-500', bg: 'bg-green-500/10' },
            { country: 'Saudi Arabia', flag: '🇸🇦', policy: 'Saudization (Nitaqat)',risk: profile.sector === 'Technology' ? 'Medium' : 'High',  ratio: '30% target', color: profile.sector === 'Technology' ? 'text-amber-500' : 'text-red-500', bg: profile.sector === 'Technology' ? 'bg-amber-500/10' : 'bg-red-500/10' },
            { country: 'Oman',         flag: '🇴🇲', policy: 'Omanisation',         risk: 'High',   ratio: 'Varies by sector',    color: 'text-red-500',   bg: 'bg-red-500/10' },
            { country: 'Qatar',        flag: '🇶🇦', policy: 'Qatarisation',        risk: 'Medium', ratio: '20% target',          color: 'text-amber-500', bg: 'bg-amber-500/10' },
            { country: 'Bahrain',      flag: '🇧🇭', policy: 'Bahrainisation',      risk: 'Medium', ratio: 'Fee-based flexibility',color: 'text-amber-500', bg: 'bg-amber-500/10' },
            { country: 'Kuwait',       flag: '🇰🇼', policy: 'Kuwaitisation',       risk: 'High',   ratio: '100% in Govt',        color: 'text-red-500',   bg: 'bg-red-500/10' },
          ].map((nat, i) => {
            const isTarget = profile.targetCountries.some(c => c.includes(nat.country) || nat.country.includes(c.split(' ')[0]));
            return (
              <div key={i} className={`bg-card border rounded-xl p-5 transition-all ${isTarget ? 'border-primary/30 ring-1 ring-primary/10' : 'border-border/50'}`}>
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-2xl">{nat.flag}</span>
                  <div>
                    <h3 className="font-semibold">{nat.country}</h3>
                    {isTarget && <span className="text-xs text-primary">Your target market</span>}
                  </div>
                </div>
