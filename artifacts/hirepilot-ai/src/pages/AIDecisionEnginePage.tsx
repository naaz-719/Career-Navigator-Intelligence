...
import AnalysisHistory from '@/components/decision-engine/AnalysisHistory';
...
export default function AIDecisionEnginePage() {
  ...
  const handleSubmit = useCallback(async (engineProfile: UserProfile) => {
    ...
    await runDecisionEngine(engineProfile, {
      ...
      onComplete: (engineResult) => {
        ...
        addEntry(engineResult);            // ← persist to localStorage history
        setTimeout(() => setStep('result'), 800);
      },
    });
  }, [updateModule, mergeFromAnalysis, setLastAnalysis, addEntry]);
  ...
  const handleRestoreHistory = useCallback((restored: DecisionResult) => {
    setResult(restored);
    setLastAnalysis(restored);
    setStep('result');
  }, [setLastAnalysis]);
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="ml-2 text-xs text-muted-foreground/60">
          {step === 'input'     && `Pre-filled from your profile · ${profile.name} · ${profile.currentRole}`}
          {step === 'reasoning' && 'AI is analysing across 6 intelligence modules…'}
          {step === 'result'    && 'Analysis complete — profile & all pages updated · saved to history'}
        </div>
      </div>
      ...
      <AnimatePresence>
        {step === 'input' && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <p className="text-sm text-muted-foreground">
              Your profile is <span className="text-primary font-medium">pre-filled</span> from your shared platform data
              ({profile.name} · {profile.sector} · {profile.yearsExperience} yrs).
              Submitting this analysis will <span className="text-foreground font-medium">update all pages</span> and save to your history.
            </p>
          </motion.div>
        )}
      </AnimatePresence>

      {/* ── Two-column layout for input step (form + history) ── */}
      {step === 'input' ? (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          {/* Form — takes 2/3 */}
          <div className="xl:col-span-2 rounded-2xl border border-border/40 bg-card/30 backdrop-blur-sm p-6 md:p-8">
            <InputForm onSubmit={handleSubmit} isLoading={isLoading} initialProfile={initialFormValues} />
          </div>

          {/* History sidebar — takes 1/3 */}
          {history.length > 0 && (
            <div className="xl:col-span-1">
              <AnalysisHistory history={history} onRestore={handleRestoreHistory} onClear={clearHistory} />
            </div>
          )}
        </div>
      ) : (
        /* Reasoning + Result — full width */
        <div className="rounded-2xl border border-border/40 bg-card/30 backdrop-blur-sm p-6 md:p-8">
          <AnimatePresence mode="wait">
            {step === 'reasoning' && (
              <motion.div key="reasoning" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <p className="text-center text-xs text-muted-foreground/50 pb-4">
        AI Decision Engine v2.0 · Analysis based on GCC market data, government policy databases, and employer hiring patterns ·
