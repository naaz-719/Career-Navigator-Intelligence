...
import {
  getWhyCareerScore,
  getWhyHireProbabilityDashboard,
  getWhyInterviewReadiness,
...
export default function DashboardPage() {
  ...
  const hirePct = lastAnalysis
    ? lastAnalysis.hireProbability
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-1 mb-2">
          ...
          <p className="text-muted-foreground text-sm">
            {profile.currentRole} · {profile.sector} · {profile.yearsExperience}{" "}
            yrs experience
          </p>
        </div>
        {lastAnalysis && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <motion.div
          variants={item}
          className="bg-card border border-border/50 rounded-xl p-5 flex flex-col justify-between"
        >
          ...
          <div className="mt-2 text-xs text-green-400 font-medium flex items-center gap-1 mb-2">
            ...
            {metrics.careerScore >= 80
              ? "Strong profile"
              : metrics.careerScore >= 65
                ? "Good standing"
                : "Needs improvement"}
          </div>
        ...
        >
          ...
          <div className="mt-2 text-xs text-green-400 font-medium flex items-center gap-1 mb-2">
            <TrendingUp className="w-3 h-3" />+
            {Math.round(
              ((metrics.salaryPotential - profile.currentSalary) /
        ...
        >
          ...
          <div className="flex items-end gap-2 mb-2">
            <span className="text-3xl font-bold text-foreground">
              {metrics.interviewReadiness}%
            </span>
          </div>
          <div className="w-full bg-muted rounded-full h-1.5 mb-2">
            <div
              className="bg-primary h-1.5 rounded-full"
              style={{ width: `${metrics.interviewReadiness}%` }}
            />
          </div>
          <div
            className={`text-xs mb-2 ${skillGaps.length > 3 ? "text-amber-400" : "text-green-400"}`}
          >
            {skillGaps.length} skill gap{skillGaps.length !== 1 ? "s" : ""} to
            close
          </div>
          <WhyPanel
            data={getWhyInterviewReadiness(profile, metrics.interviewReadiness)}
          />
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <motion.div
          variants={item}
          className="lg:col-span-2 bg-card border border-border/50 rounded-xl p-5"
        >
          ...
          <p className="text-xs text-muted-foreground mb-5">
            Current path (AED {(profile.currentSalary / 1000).toFixed(0)}K/mo
            base) vs recommended GCC move
          </p>
          ...
          <div className="space-y-4">
            {skillGaps.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Great! Your skills cover all key market demands for{" "}
                {profile.sector}.
              </p>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="space-y-3">
            {jobs.map((job, i) => (
              <div
                key={i}
                className="flex flex-col p-3 rounded-lg border border-border/30 hover:bg-muted/20 transition-colors gap-2"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right flex flex-col items-end gap-1">
                    ...
                    <span
                      className={`text-xs font-bold ${job.match >= 85 ? "text-emerald-400" : job.match >= 70 ? "text-amber-400" : "text-red-400"}`}
                    >
                      {job.match}% match
                    </span>
                  </div>
        ...
        {/* Recent Analyses — real history, not fabricated */}
        <motion.div
          variants={item}
          className="bg-card border border-border/50 rounded-xl p-5"
        >
          ...
          <div className="space-y-3">
            {history.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                You haven't run an analysis yet — ask the AI Decision Engine a
                real question to see your results here.
              </p>
            ) : (
              history.slice(0, 3).map((entry) => (
                <Link key={entry.id} href="/ai-decision-engine">
                  <div className="p-3 rounded-lg border border-border/30 hover:border-primary/30 transition-colors cursor-pointer group">
                    <div className="flex justify-between items-start mb-2">
                      <span className="font-medium text-sm group-hover:text-primary transition-colors truncate pr-2">
                        {entry.question}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-lg font-bold ${entry.hireProbability > 70 ? "text-green-500" : entry.hireProbability > 55 ? "text-amber-500" : "text-red-500"}`}
                      >
                        {entry.hireProbability}%
                      </span>
                      <span className="text-xs text-muted-foreground">
                        Hire probability · {entry.topCountry}
                      </span>
                    </div>
                  </div>
                </Link>
            ...
            <Link href="/ai-decision-engine">
              <div className="p-3 rounded-lg border border-dashed border-primary/30 hover:border-primary/60 hover:bg-primary/5 transition-colors cursor-pointer text-center">
                <span className="text-xs text-primary font-medium flex items-center justify-center gap-2">
                  <Brain className="w-3.5 h-3.5" /> Run AI Decision Engine
                  Analysis
                </span>
              </div>
            </Link>
