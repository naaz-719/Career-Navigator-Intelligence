  ...
  return (
    <div className="flex items-center gap-3 flex-wrap text-[10px]">
      ...
      <span className="text-muted-foreground/40 italic">
        All numbers are model-estimated, not live market data
      </span>
    </div>
  );
}
...
function CountryRow({
  ...
  c: DecisionResult["countryAnalysis"][0];
  rank: number;
}) {
  const shortName = c.country
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 pr-24">
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            <span
              className={`text-xs px-2 py-0.5 rounded-full border font-medium ${RISK_COLORS[c.visaFeasibility]}`}
            >
              Visa: {c.visaFeasibility}
            </span>
            <span
              className={`text-xs px-2 py-0.5 rounded-full border font-medium ${RISK_COLORS[c.nationalizationRisk === "Low" ? "Low" : c.nationalizationRisk === "Medium" ? "Medium" : "High"]}`}
            >
            ...
            <span className="text-xs px-2 py-0.5 rounded-full border border-border/40 text-muted-foreground">
              {c.jobCount.toLocaleString()} open roles
            </span>
          </div>
          ...
          <div className="mt-2 flex items-center gap-1.5 flex-wrap">
            ...
            {c.topEmployers.slice(0, 3).map((e) => (
              <span key={e} className="text-xs text-muted-foreground/70">
                {e}
              </span>
            ))}
            {c.topEmployers.length > 3 && (
              <span className="text-xs text-muted-foreground/50">
                +{c.topEmployers.length - 3} more
              </span>
            )}
          </div>
          ...
          <Link href="/jobs">
            <span className="mt-3 inline-flex items-center gap-1 text-xs text-primary hover:underline font-medium cursor-pointer">
              <Briefcase className="h-3 w-3" /> View{" "}
              {c.jobCount.toLocaleString()} jobs in {shortName}{" "}
...
function InsightAccordion({
  ...
  insights: DecisionResult["moduleInsights"];
}) {
          ...
          >
            ...
            >
              <div className="flex items-center gap-2 text-left">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full border font-medium ${MODULE_COLORS[ins.moduleId]}`}
                >
...
function SalaryAnchorPanel({
  anchor,
  profile,
}: {
  anchor: DecisionResult["salaryAnchor"];
  profile: DecisionResult["profile"];
}) {
      ...
      <div className="space-y-3">
        <div className="relative h-3 rounded-full bg-muted/50 overflow-hidden">
          ...
          <div
            ...
            style={{
              width: `${(anchor.walkaway / (anchor.askPrice * 1.2)) * 100}%`,
            }}
          />
          {/* Target zone */}
          <div
        ...
        <div className="grid grid-cols-4 gap-2 text-center">
          {[
            {
              label: "Walkaway",
              value: anchor.walkaway,
              color: "text-red-400",
            },
            {
              label: "Expected Offer",
              value: anchor.expectedOffer,
              color: "text-amber-400",
            },
            {
              label: "Your Target",
              value: anchor.targetPrice,
              color: "text-primary",
            },
            {
              label: "Open Ask",
              value: anchor.askPrice,
      ...
      <div className="p-5 rounded-xl border border-amber-500/20 bg-amber-500/5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-amber-400">
            Risks & Watch Points ({risks.length})
          </h3>
        </div>
        <ul className="space-y-2.5">
          {risks.map((r, i) => (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-3 mb-1">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full border font-medium ${urg.badge}`}
                  >
                    {step.urgency}
                  </span>
...
function CountryRadarChart({
  ...
  countries: DecisionResult["countryAnalysis"];
}) {
...
const QUICK_ACTIONS = [
  ...
  {
    label: "Career Multiverse",
    href: "/career-multiverse",
    icon: Zap,
    color: "text-blue-400",
  },
];
...
export default function ResultPanel({ result, onReset }: Props) {
  const salaryDelta = Math.round(
    ((result.profile.targetSalary - result.profile.currentSalary) /
      result.profile.currentSalary) *
      100,
  );
    ...
    >
        ...
        <div className="relative">
          ...
          <div className="mt-3 flex items-center gap-3 flex-wrap">
            <p className="text-xs text-muted-foreground/50">
              ...
              ·{result.profile.nationality} · {result.profile.currentRole} ·{" "}
              {result.profile.yearsExperience} yrs · {result.profile.sector}
            </p>
          </div>
          <div className="mt-2">
            <DataLegend />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="col-span-2 row-span-1 p-6 rounded-xl border border-border/40 bg-card/50 flex flex-col items-center justify-center gap-3">
          ...
          <WhyPanel
            data={getWhyHireProbabilityEngine(result)}
            trigger="badge"
          />
        </div>
        ...
        <div className="p-5 rounded-xl border border-border/40 bg-card/50 flex flex-col gap-1">
          ...
          <p className="text-xs text-muted-foreground/70 mt-1 mb-2">
            Profile completeness + historical match accuracy
          </p>
        ...
        <div className="p-5 rounded-xl border border-border/40 bg-card/50 flex flex-col gap-1">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Highest hire probability + salary feasibility
          </p>
        </div>
        ...
        <div className="p-5 rounded-xl border border-border/40 bg-card/50 flex flex-col gap-1">
          ...
          <div className="flex items-end gap-1.5 mt-1">
            <span
              className={`text-3xl font-bold ${salaryDelta > 0 ? "text-emerald-400" : "text-foreground"}`}
            >
              {salaryDelta > 0 ? "+" : ""}
              {salaryDelta}%
            </span>
          ...
          <p className="text-xs text-muted-foreground mt-1 mb-2">
            AED {(result.profile.currentSalary / 1000).toFixed(0)}K →{" "}
            {(result.profile.targetSalary / 1000).toFixed(0)}K/mo ·{" "}
            <span
              className={
                result.salaryFeasibility === "High"
                  ? "text-emerald-400"
                  : result.salaryFeasibility === "Medium"
                    ? "text-amber-400"
                    : "text-red-400"
              }
            >
          ...
          <WhyPanel
            data={getWhySalaryPotential(result.profile)}
            trigger="badge"
          />
        </div>
        ...
        <div className="p-5 rounded-xl border border-border/40 bg-card/50 flex flex-col gap-1">
          ...
          <div className="flex items-end gap-1.5 mt-1">
            <span className="text-3xl font-bold text-foreground">
              {result.matchedJobCount.toLocaleString()}
            </span>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
        ...
        <div className="p-5 rounded-xl border border-border/40 bg-card/50 flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-xl font-bold text-foreground">
              {result.timeToHireEstimate}
            </span>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            {result.profile.sector} sector · GCC market average
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Click each module to expand the detailed insight
            </p>
          </div>
        </div>
      ...
      <section>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {result.countryAnalysis.length} GCC markets analysed — ranked by
              hire probability
            </p>
          </div>
        </div>
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
          <div className="xl:col-span-3 space-y-3">
            {result.countryAnalysis.map((c, i) => (
          ...
          <div className="xl:col-span-2 p-4 rounded-xl border border-border/30 bg-card/40">
            ...
            <div className="flex items-center justify-center gap-4 mt-2 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-primary" /> Hire %
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-accent" /> Salary
              </span>
              <span className="flex items-center gap-1">
                <span
      ...
      <section>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Profile-specific factors affecting your outcome
            </p>
          </div>
        </div>
      ...
      <section>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Prioritised by urgency × ROI — click actions to navigate
            </p>
          </div>
        </div>
